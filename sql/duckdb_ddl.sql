-- Arbibet capstone -- DuckDB schema (the local warehouse).
--
-- Ported from snowflake/ddl.sql, which stays as the record of the Snowflake
-- deployment. Three schemas:
--   core       written by the pipeline
--   analytics  written by dbt
--   ops        the runner's own observability: run log, heartbeats, freshness
--
-- NO PRIMARY OR FOREIGN KEY CONSTRAINTS, deliberately. Snowflake never
-- enforced them, the data was written under that rule, and DuckDB would both
-- enforce them (so a historical duplicate fails the copy) and keep an in-memory
-- index per key (hundreds of MB for fact_team_market_result's 6.2M rows). Keys
-- are stated in comments; idempotency is the MERGE ON clause, as before, and
-- dbt tests still check uniqueness.
...
CREATE TABLE IF NOT EXISTS dim_fixture (
    ...
    apifootball_id  BIGINT,            -- null until the matcher resolves one
...
CREATE TABLE IF NOT EXISTS dim_market (
    market_base_id     BIGINT NOT NULL,  -- betradar market id
    market_name        VARCHAR      NOT NULL,
    bet9ja_key         VARCHAR,                -- null => bet9ja does not price it
    livescorebet_type  BIGINT,           -- null => livescorebet does not price it
...
CREATE TABLE IF NOT EXISTS fact_arbitrage_signal (
    ...
    market_base_id      BIGINT NOT NULL,  -- joins dim_market
    specifier           VARCHAR(32),            -- '2.5', '0:1', null when the market has none

    arbitrage           DOUBLE        NOT NULL,  -- 1/sum(1/odds); > 1 is a surebet
    n_legs              BIGINT NOT NULL,
    legs                JSON      NOT NULL,  -- [{outcome_id, bookmaker, odds}, ...]
    ...
    oldest_leg_fire_time TIMESTAMPTZ NOT NULL,
    newest_leg_fire_time TIMESTAMPTZ NOT NULL,
    leg_spread_seconds   BIGINT NOT NULL,   -- refused above 300 at detection
...
CREATE TABLE IF NOT EXISTS fact_arbitrage_track (
    ...
    observed_at           TIMESTAMPTZ NOT NULL,   -- the payload that changed the picture
    arbitrage             DOUBLE,
    leg_spread_seconds    BIGINT,
    newest_leg_fire_time  TIMESTAMPTZ,
    loaded_at             TIMESTAMPTZ NOT NULL DEFAULT current_timestamp
...
CREATE TABLE IF NOT EXISTS fact_event_state (
    ...
    fired_at      TIMESTAMPTZ,     -- the payload this was read from
...
CREATE TABLE IF NOT EXISTS fact_ev_signal (
    ...
    event_id         VARCHAR(36)  NOT NULL,
    market_id        VARCHAR(64)  NOT NULL,
    market_base_id   BIGINT NOT NULL,
    ...
    implied_p        DOUBLE        NOT NULL,   -- the p used in the EV calculation
    p_source         VARCHAR(32)  NOT NULL,   -- which book supplied that p; provenance matters
    ev               DOUBLE        NOT NULL,   -- implied_p * odds - 1

    payload_fire_time TIMESTAMPTZ NOT NULL,          -- when the PRICE was published
    ...
    probability_fire_time      TIMESTAMPTZ,
    probability_spread_seconds BIGINT,         -- |price - probability|; refused above 300
    detected_at      TIMESTAMPTZ NOT NULL DEFAULT current_timestamp,  -- later of the two
    consumed_at      TIMESTAMPTZ                                     -- when a consumer processed it
...
CREATE TABLE IF NOT EXISTS fact_team_match (
    ...
    team_id         BIGINT NOT NULL,   -- API-Football team id
    ...
    score_halftime_for      BIGINT,
    score_halftime_against  BIGINT,
    score_fulltime_for      BIGINT,
    score_fulltime_against  BIGINT,
    score_extratime_for     BIGINT,
    score_extratime_against BIGINT,
    score_penalty_for       BIGINT,
    score_penalty_against   BIGINT,

    -- D18 period components, derived at load time; the settlement basis
    goals_for_h1        BIGINT,
    goals_for_h2        BIGINT,
    goals_for_reg       BIGINT,
    goals_for_et        BIGINT,
    goals_for_pens      BIGINT,
    goals_for_full      BIGINT,
    goals_against_h1    BIGINT,
    goals_against_h2    BIGINT,
    goals_against_reg   BIGINT,
    goals_against_et    BIGINT,
    goals_against_pens  BIGINT,
    goals_against_full  BIGINT,
    ...
    xg                              DOUBLE,
    xg_against                      DOUBLE,
    goals_prevented_fixture         DOUBLE,   -- fixture-level; see note above
    goals_against_minus_xg_against  DOUBLE,   -- the per-team keeper signal (D16)
    ...
    saves           BIGINT,
    passes_total    BIGINT,
    passes_accurate BIGINT,            -- a COUNT (D16)
...
-- STORED OUTSIDE THE DATABASE, as zstd Parquet under data/lake/, and exposed
-- as the view `core.bronze_slip_payload` (created by warehouse.py, which knows
-- the path). Each slip version is ~110 KB of JSON; DuckDB keeps strings that
-- long uncompressed, and 18k of them made a 3.9 GB file. The same rows as
-- Parquet are 4 MB. Columns:
--   source, share_code, payload_hash, payload JSON, list_id, followed_times,
--   folds, first_fetched_at, last_fetched_at
-- Append-only: a re-fetched version appends a row, and the view keeps the
-- newest per (source, share_code, payload_hash) with the earliest
-- first_fetched_at, which is what the Snowflake MERGE converged to.
...
CREATE TABLE IF NOT EXISTS gold_slip_summary_ai (
    ...
    legs_with_history BIGINT NOT NULL, -- how much evidence it was given
...
CREATE TABLE IF NOT EXISTS gold_fixture_summary_ai (
    ...
    markets_described   BIGINT,
    sides_with_history  BIGINT,
...
CREATE TABLE IF NOT EXISTS gold_fixture_result_ai (
    ...
    markets_shown  BIGINT,
    signals_shown  BIGINT,
...
CREATE TABLE IF NOT EXISTS pipeline_cursor (
    ...
    position   TIMESTAMPTZ NOT NULL,   -- the newest input already handled
...
-- ---------------------------------------------------------------- ops
-- What the Airflow UI used to show, kept by the runner itself.

-- One row per job execution.
CREATE TABLE IF NOT EXISTS ops.job_run (
    run_id        VARCHAR      NOT NULL,   -- uuid
    job           VARCHAR      NOT NULL,   -- 'hot.signals', 'warm.dbt', ...
    loop          VARCHAR      NOT NULL,   -- hot | warm | cold
    trigger       VARCHAR,                 -- notify | poll | schedule | manual
    started_at    TIMESTAMPTZ  NOT NULL,
    finished_at   TIMESTAMPTZ,
    status        VARCHAR      NOT NULL,   -- running | ok | failed | skipped
    rows_written  BIGINT,
    detail        JSON,                    -- job-specific counts
    error         VARCHAR
);

-- One row per runner component, overwritten on every beat.
CREATE TABLE IF NOT EXISTS ops.heartbeat (
    component     VARCHAR      NOT NULL,   -- 'runner', 'listener', 'hot', 'warm', 'cold'
    beat_at       TIMESTAMPTZ  NOT NULL,
    state         VARCHAR,                 -- idle | working | waiting | error
    detail        JSON
);

-- ---------------------------------------------------------------- seed

-- Idempotent: the file is re-run on every start.
INSERT INTO core.dim_bookmaker
SELECT * FROM (VALUES
...
) v (bookmaker_id, bookmaker_name, is_betradar_native, publishes_probability)
WHERE NOT EXISTS (SELECT 1 FROM core.dim_bookmaker);
