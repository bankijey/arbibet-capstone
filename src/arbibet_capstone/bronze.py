...
def latest_payloads(conn: psycopg.Connection, event_id: UUID) -> dict[str, bytes]:
    ...
    Keys are parser-registry names. Values are the verbatim response bytes,
    undecoded: bronze's contract is byte fidelity, and a malformed body should
    fail in the parser, where that failure has a reason code, rather than here.
    ...
    with conn.cursor() as cur:
        ...
        return {to_parser_name(book): payload for book, payload in cur}
